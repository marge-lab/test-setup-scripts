#!/usr/bin/env python3
"""
Web eID .NET naiterakenduse paigaldus — haru-test (Windows / macOS).

Kasutus:
   Windows:  setup-web-eid-dotnet-branch.cmd                       (interaktiivne haru-valik)
             setup-web-eid-dotnet-branch.cmd --branch HARU         (otse antud haru)
             python setup-web-eid-dotnet-branch.py [--branch HARU] [--profile dev|prod]
   macOS:    python3 setup-web-eid-dotnet-branch.py [--branch HARU] [--profile dev|prod]

Erinevus main-haru skriptist (setup-web-eid-dotnet.py):
  - Kasutaja saab valida haru (--branch arg voi interaktiivne menuu viimase 10 hulgast)
  - WebEid.Security ehitatakse LOKAALSE NuGet-paketi-na versioonis 1.2.0-beta1
    (eristub GitLab-i 1.2.0-st, et iga harutestimine kasutaks oma versiooni)
  - example .csproj kasutab PackageReference (lokaalsele NuGet-le), mitte
    ProjectReference (nagu main-haru skriptis)

Skript:
  1. .NET 8 SDK
  2. Git
  3. libdigidocpp (DigiDoc4 paigaldusest voi GitHub-MSI)
  4. Repo kloonimine + haru valimine
  5. Lokaalne NuGet build (WebEid.Security 1.2.0-beta1)
  6. example .csproj patch (PackageReference 1.2.0-beta1)
  7. C# bindings (.cs failid) projektisse
  8. HTTPS dev-sertifikaat
  9. Test-TSL flag (--profile dev) voi digidocpp.conf (--profile prod)
 10. Ehitamine
 11. Natiivteegid bin/ + kaivitus

Soltuvused: ainult Python stdlib.
"""
...
# Iga harutestimine ehitab WebEid.Security selle versiooniga.
...
def step_libdigidocpp() -> None:
    step(3, 11, "libdigidocpp (dev-teek, MITTE DigiDoc4 Client)")
...
def _list_recent_branches() -> list:
    """Tagasta viimati uuendatud 10 origin/* haru (peale main + HEAD)."""
    result = subprocess.run(
...
def step_clone_repo() -> None:
    step(4, 11, "Repo kloonimine + haru valimine")
    ...
    # Haru valimine
    if BRANCH_ARG:
        # substring-match: kui antud "1234", proovi leida "WE2-1234-XYZ" tuupi haru
        result = subprocess.run(
...
# --- 5. Lokaalne NuGet build (WebEid.Security 1.2.0-beta1) ------------------
def step_local_nuget_build() -> None:
    """Ehita WebEid.Security NuGet-paketiks NUGET_VERSION versioonis.

    GeneratePackageOnBuild on csproj-is true, niisiis `dotnet build --configuration
    Release -p:Version=X -p:PackageVersion=X` toodab automaatselt .nupkg-i
    bin/Release/-i. Kopeerime selle LOCAL_NUGET kausta ja registreerime
    dotnet nuget allikana.
    """
    step(5, 11, f"Lokaalne NuGet build (WebEid.Security {NUGET_VERSION})")
    ...
    # Puhasta vana obj/bin valitud haru jaoks
    ...
    # Eemalda vana allikas (kui olemas) ja lisa uus
    subprocess.run(
...
def step_patch_csproj() -> None:
    step(6, 11, f"WebEid.Security viide → PackageReference {NUGET_VERSION}")
    ...
    if target in content:
        info(f"OK: {CSPROJ.name} juba kasutab PackageReference {NUGET_VERSION}")
        return

    import re
    # Kaks juhtu kaetud:
    #   (a) <ProjectReference ... WebEid.Security.csproj /> — main-haru
    #   (b) <PackageReference Include="WebEid.Security" Version="X.Y.Z" /> — upstream
    ...
    if count_p + count_k == 0:
        fail(
            f"Ei suutnud {CSPROJ.name}-is WebEid.Security viidet asendada. "
            "Toenaoliselt upstream-i vorming muutunud — vaata kasitsi."
        )
...
def step_copy_cs_bindings() -> None:
    ...
    """
    step(7, 11, "libdigidocpp C# bindings (.cs failid) projektisse")
...
def step_dev_cert() -> None:
    step(8, 11, "HTTPS dev-sertifikaat (dotnet dev-certs)")
...
def step_tsl_flag() -> None:
    if PROFILE == "prod":
        step(9, 11, "TSL config (prod-profile — EE_T.xml ei vaja)")
    ...
    step(9, 11, "Test-TSL flag (EE_T.xml)")
...
def step_build() -> None:
    step(10, 11, "Ehitamine (dotnet restore + build)")
...
def step_copy_native_libs_and_run() -> None:
    ...
    """
    step(11, 11, "libdigidocpp natiivteegid build output-i + kaivitamine")
...
def main() -> None:
    ...
    print(f"{G}=== Web eID .NET naiterakenduse paigaldus (haru-test) ==={N}")
    print(f"  Profile:  {PROFILE}  ({profile_label})")
    if BRANCH_ARG:
        print(f"  Haru:     {BRANCH_ARG}  (--branch argumendist)")
    else:
        print(f"  Haru:     valitakse interaktiivselt sammus 4")
